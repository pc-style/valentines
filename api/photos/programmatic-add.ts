...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!username || !ALLOWED_USERS.includes(username)) {
    return res
      .status(400)
      .json({ error: "Invalid or missing username header (x-username)" });
  }

  if (key !== process.env.REGISTRATION_KEY) {
    return res.status(401).json({ error: "Invalid API key (x-api-key)" });
  }
  ...
  try {
    ...
    const result = await sql`
      ...
      VALUES (${blob.url}, ${date}, ${message}, ${section}, ${username}, ${sortOrder})
